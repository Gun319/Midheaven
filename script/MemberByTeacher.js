        ...
        success: (data) => {
            ...
            $.each(data, (idnex, item) => {
                str += '<div class="courseItem shadow-sm mt-3"><div class="img">' +
                    ...
                    '</label></div><div class="inline-info2"><span class="title">变更可选状态：</span><button class="btn btn-primary" onclick="Updete(' + item["name"]["C_ID"] + ');">' + (item["name"]["C_flog"] == 0 ? "可选" : "维护中") + '</button></div>' +
                    '<div class="inline-info2"><button class="btn btn-danger" onclick="EditByFlog(' + item["name"]["C_ID"] + ');">清除该课程</button></div></div >';
            })
